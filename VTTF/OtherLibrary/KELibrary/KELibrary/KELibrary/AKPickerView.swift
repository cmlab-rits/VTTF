...
public class AKPickerView: UIView, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, AKCollectionViewLayoutDelegate {
	...
	*/
	private func initialize() {
		...
		self.collectionView.register(AKCollectionViewCell.self, forCellWithReuseIdentifier: NSStringFromClass(AKCollectionViewCell.self))
        
        self.centerView = UIView()
        self.centerView.layer.cornerRadius = 8.0
        self.centerView.backgroundColor = UIColor.white.withAlphaComponent(0.3)
	...
	public override func layoutSubviews() {
        ...
        self.centerView.frame = CGRect(x: self.collectionView.frame.width/2.0 - 20.0, y: 0.0, width: 40.0, height: self.collectionView.frame.height)
	}
	...
	*/
	private func selectItem(item: Int, animated: Bool, notifySelection: Bool) {
        self.collectionView.selectItem(at: IndexPath(item: item, section: 0), animated: animated, scrollPosition: [])
	...
	*/
	...
	public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
		let cell = collectionView.dequeueReusableCell(withReuseIdentifier: NSStringFromClass(AKCollectionViewCell.self), for: indexPath as IndexPath) as! AKCollectionViewCell
		if let title = self.dataSource?.pickerView?(pickerView: self, titleForItem: indexPath.item) {
			...
			if let delegate = self.delegate {
				...
				if let margin = delegate.pickerView?(pickerView: self, marginForItem: indexPath.item) {
					cell.label.frame = cell.label.frame.insetBy(dx: -margin.width, dy: -margin.height) //CGRectInset(cell.label.frame, -margin.width, -margin.height)
				}
			}
