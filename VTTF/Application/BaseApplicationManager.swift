...
class BaseApplicationManager: NSObject {
   ...
   // private let socketManager = SocketManager.sharedInstance
    ...
    private let openCVManager = OpenCVManager.sharedInstance
    ...
    var basisWorkspaceSize: CGSize = CGSize(width: 4000, height: 4000)
    ...
    func recaptureInitialPosition() {
        openCVManager.recaptureInitialPosition()
    }
...
extension BaseApplicationManager: OpenCVManagerDelegate {
    func manager(manager: OpenCVManager, scrollBy move: (Float, Float)) {
        if let currentSelectedLabel = currentSelectedLabel {
            currentSelectedLabel.frame.origin.x += CGFloat(move.0*7.0)
            currentSelectedLabel.frame.origin.y += CGFloat(move.1*7.2)
        }
